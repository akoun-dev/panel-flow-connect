...
import { 
  Calendar, Users, MessageSquare, Clock, Plus, Settings, BarChart3, 
  Play, Edit, Trash2, Eye, Mail, Copy, Share2, Download, Archive,
  Zap, TrendingUp, Award, RefreshCw, MoreHorizontal, Filter,
  Grid, List, Search, X, CheckCircle, AlertCircle, PieChart,
  ExternalLink, QrCode, Send, Bell, Star, Shield
} from "lucide-react";
...
export function UserDashboard() {
    ...
    const filteredPanels = useMemo(() => {
        return userData.upcomingPanels.filter(panel => {
            const matchesStatus = filterStatus === 'all' || panel.status === filterStatus;
    ...
    const panelStats = useMemo(() => {
        ...
        const byStatus = {
            live: userData.upcomingPanels.filter(p => p.status === "live").length,
            scheduled: userData.upcomingPanels.filter(p => p.status === "scheduled").length,
            completed: userData.upcomingPanels.filter(p => p.status === "completed").length,
            draft: userData.upcomingPanels.filter(p => p.status === "draft").length,
        };
        const totalQuestions = userData.upcomingPanels.reduce((sum, p) => sum + (p.questions || 0), 0);
    ...
    const updatePanelStatus = useCallback(async (panelId: string, newStatus: 'draft' | 'scheduled' | 'live' | 'completed' | 'cancelled') => {
        try {
    ...
    // Actions groupées
    const handleBulkAction = useCallback((action: 'export' | 'archive' | 'delete') => {
    ...
    }, [user, userProfile, panelForm, editingPanelId, handleCloseModal, fetchData]);

    // Composants
    const getPriorityBadge = (panel: Panel) => {
        if (panel.status === 'live') return { label: "En direct", color: "bg-red-100 text-red-700", icon: Zap };
        if ((panel.questions || 0) >= 10) return { label: "Actif", color: "bg-blue-100 text-blue-700", icon: TrendingUp };
        return null;
    };
    ...
    return (
        <>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div className="flex items-center gap-4 text-sm text-muted-foreground mb-3">
                                        <span className="flex items-center gap-1">
                                            <Calendar className="h-4 w-4" />
                                            {new Date(userData.nextPanel.date).toLocaleDateString("fr-FR")}
                                        </span>
                                        <span className="flex items-center gap-1">
                        ...
                        <Card className="border-0 shadow-lg">
                            <CardContent className="text-center py-12">
                                ...
                                <h3 className="text-lg font-semibold text-gray-900 mb-2">
                                    {searchTerm || filterStatus !== 'all' ? 'Aucun panel trouvé' : 'Aucun panel à venir'}
                                </h3>
                ...
                <Dialog open={managePanelModal.open} onOpenChange={(open) => 
                    ...
                    <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
                        <DialogHeader>
                            <div className="flex items-center justify-between">
                                <div>
                                    <DialogTitle className="text-xl">Gestion du Panel</DialogTitle>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div className="md:col-span-2 space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            <Label className="text-sm font-medium text-muted-foreground">Panélistes</Label>
                                            <div className="flex items-center gap-2 mt-1">
                                                <Users className="h-4 w-4 text-muted-foreground" />
                                                <span className="font-semibold">{managePanelModal.panel.panelists?.length || 0}</span>
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <div className="flex justify-center mt-2">
                                            <PanelQRCode panel={managePanelModal.panel} size={150} />
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="border-t pt-6">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                    ...
                                    <Button
                                        variant="outline"
                                        size="sm"
                                        onClick={() => handleSharePanel(managePanelModal.panel!)}
                                        className="flex items-center gap-2"
                                    >
                                        <Share2 className="h-4 w-4" />
                                        Partager
                                    ...
                                    <Button
                                        variant="outline"
                                        size="sm"
                                        onClick={() => handleExportPanel(managePanelModal.panel!)}
                                        className="flex items-center gap-2"
                                    >
                                    ...
                                    <Button
                                        variant="outline"
                                        size="sm"
                                        onClick={() => handleDuplicatePanel(managePanelModal.panel!)}
                                        className="flex items-center gap-2"
                                    >
                            ...
                            <div className="border-t pt-6">
                                ...
                                <div className="flex gap-3">
                                    ...
                                    ) : managePanelModal.panel.status === 'scheduled' ? (
                                        <>
                                            <Button
                                                variant="default"
                                                size="sm"
                                                onClick={() => updatePanelStatus(managePanelModal.panel!.id, 'live')}
                                            >
                                                <Play className="h-4 w-4 mr-2" />
                                                Démarrer le panel
                                            </Button>
                                    ...
                                    ) : managePanelModal.panel.status === 'live' ? (
                                        <Button
                                            variant="destructive"
                                            size="sm"
                                            onClick={() => updatePanelStatus(managePanelModal.panel!.id, 'completed')}
                                        >
                                            <CheckCircle className="h-4 w-4 mr-2" />
                                            Terminer le panel
                                        </Button>
                                    ) : (
                                        <p className="text-sm text-gray-500">Panel terminé</p>
                                    )}
                                </div>
                            </div>
                        </div>
            ...
            {shareModal.open && shareModal.panel && (
                <Dialog open={shareModal.open} onOpenChange={(open) => 
                    setShareModal({ open, panel: null })
                }>
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                <div className="text-center">
                                    ...
                                    <div className="bg-white p-4 rounded-lg border inline-block">
                                        <PanelQRCode panel={shareModal.panel} size={200} />
                                    </div>
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <div className="flex mt-1">
                                            ...
                                            <Button 
                                                ...
                                                onClick={() => handleCopyLink(shareModal.panel!)}
                                            >
                                                <Copy className="h-4 w-4" />
                                            </Button>
                                        </div>
                                    ...
                                    <div className="space-y-2">
                                        <Button 
                                            className="w-full"
                                            onClick={() => {
                                                window.open(`mailto:?subject=Invitation au panel: ${shareModal.panel!.title}&body=Vous êtes invité à participer au panel "${shareModal.panel!.title}". Accédez-y ici: ${window.location.origin}/panel/${shareModal.panel!.id}`);
                                            }}
                                        >
                                            <Mail className="h-4 w-4 mr-2" />
                                            Partager par email
                                        ...
                                        <Button 
                                            ...
                                            onClick={() => {
                                                if (navigator.share) {
                                                    navigator.share({
                                                        title: shareModal.panel!.title,
                                                        text: `Participez au panel: ${shareModal.panel!.title}`,
                                                        url: `${window.location.origin}/panel/${shareModal.panel!.id}`
                                                    });
                                                }
                                            }}
                                        >
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => {
                                            setPanelForm({...panelForm, category: e.target.value});
                                            setHasUnsavedChanges(true);
                                        }}
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea
                                    ...
                                    onChange={(e) => {
                                        setPanelForm({...panelForm, description: e.target.value});
                                        setHasUnsavedChanges(true);
                                    }}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => {
                                            setPanelForm({...panelForm, date: e.target.value});
                                            setHasUnsavedChanges(true);
                                        }}
                                    />
                                </div>
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => {
                                            setPanelForm({...panelForm, time: e.target.value});
                                            setHasUnsavedChanges(true);
                                        }}
                                    />
                                </div>
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => {
                                            setPanelForm({...panelForm, duration: parseInt(e.target.value) || 60});
                                            setHasUnsavedChanges(true);
                                        }}
                                        min="15"
                                        max="480"
                            ...
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => {
                                        setPanelForm({...panelForm, participants_limit: parseInt(e.target.value) || 30});
                                        setHasUnsavedChanges(true);
                                    }}
                                    min="1"
                                    max="1000"
